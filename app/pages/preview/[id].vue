    ...
    <div v-if="article">
      ...
      <div class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
        <div class="flex items-center">
          <Icon name="mingcute:eye-line" class="w-5 h-5 text-amber-600 mr-2" />
          <span class="text-amber-800 font-medium">Aperçu de l'article</span>
        </div>
        <p class="text-amber-700 text-sm mt-1">
          Cet article est en cours de rédaction et n'est pas encore publié.
        </p>
      </div>
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <p class="text-gray-500" v-else>Brouillon</p>
        <!-- Edit button - only visible when connected and authorized -->
        <UButton 
          v-if="user && canEdit" 
...
const canEdit = computed(() => {
  ...
  // Check if user is the author, editor, or admin
  // Note: You might need to fetch user roles from your database
...
const { getArticleByIdForPreview } = useDb()

const fetchArticle = async () => {
  try {
    loading.value = true
    const found = await getArticleByIdForPreview(id)
    if (!found) {
      article.value = null
      return
    }
...
const setupSEO = (articleData: ArticleWithCategories) => {
  const title = `${articleData.title} - Aperçu | Sursaut!`
  const description = articleData.description || 
    `Aperçu de "${articleData.title}" sur Sursaut!, le journal des luttes de Bordeaux.`
  ...
  useSeoMeta({
    title,
    description,
    keywords: `${categories}, Bordeaux, luttes sociales, politique, journal, aperçu`,
    author: 'Sursaut!',
